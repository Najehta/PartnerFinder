...
@periodic_task(run_every=(crontab(minute=0, hour=7, day_of_week='sun')),
               name="add_bsfcalls_to_db", ignore_result=True)
...
@periodic_task(run_every=(crontab(minute=0, hour=7, day_of_week='sun')),
               name="add_isfcalls_to_db", ignore_result=True)
...
@periodic_task(run_every=(crontab(minute=0, hour=7, day_of_week='sun')),
               name="add_technioncalls_to_db", ignore_result=True)


def add_technioncalls_to_db():

    """
    automatic task that executes every week on sunday at 7:00 AM to update Technion calls
    :return:
    """
...
@periodic_task(run_every=(crontab(minute=0, hour=7, day_of_week='sun')),
               name="add_eucalls_to_db", ignore_result=True)

def add_eucalls_to_db():

    """
    automatic task that executes every week on sunday at 7:00 AM to update EU calls
    :return:
    """
